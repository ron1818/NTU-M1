...
class Digole(object):
    ...
    def __init__(self, bus, addr, width, height, freq=I2C_STD): 
        """ initial data, not used"""
        self.bus = mraa.I2c(bus, raw=True) # force manual bus selection
        # self.I2C_ADDRESS = addr
        # self.freq = freq
        self.bus.frequency(freq)
    ...
    def write_command(self, command_string, *values):
        """ write command string to the register """
        # convert command_string to a list of hex data
        command_string_ascii = [ord(i) for i in command_string]
        value_ascii = list()
        if values is not None:
            for value in values:

                if type(value) is str: # string value
                    value_ascii += [ord(i) for i in value]
                    # value_ascii.append(0x00)
                elif type(value) is int: # integer value
                    value_ascii.append(value)
            value_ascii.append(0x00)

        # i2c write
        self.bus.write(self.COMMAND_REG,
                command_string_ascii+value_ascii)
        # debug
        print command_string_ascii + value_ascii
...
if __name__ == "__main__":
     OLED = Digole(6, 0x27, width=160, height=128)
     OLED.clearScreen()
     OLED.setFont(6)
